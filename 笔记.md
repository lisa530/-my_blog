...
-   **获取：**    
  ...
  ```
  通过request对象获取。
      request.form.get() # 从表单表获取cookie
      request.args.get() # 从路径中获取cookie
      从请求对象request获取
...
2. ### session方式：
   ...
   注意：
   使用session必须要在配置文件中添加SECRET_KEY='xxxxx'，
   ...
   ​           把session当成字典使用, session[key]=value
  ...
  > ​     其实是如果使用session获取内容,底层会自动获取cookie中的sessionid值，
   ...
   -    **删除session**
        ...
        session.clear()  删除内存空间和浏览器的cookie
